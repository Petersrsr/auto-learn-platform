...
case 'getcount':
	...
	foreach($types as $row){
		...
		$orderrow = $DB->getRow("SELECT COUNT(*) allnum,COUNT(IF(status>0, 1, NULL)) sucnum FROM pre_order WHERE date='$today' AND channel='$channel'");
...
case 'edit_info':
	...
	if($conf['verifytype']==1){
		...
		$sqs = $DB->update('user', ['email'=>$email, 'qq'=>$qq, 'url'=>$url, 'keylogin'=>$keylogin, 'refund'=>$refund, 'transfer'=>$transfer, 'remain_money'=>$remain_money], ['uid'=>$uid]);
	}else{
		$sqs = $DB->update('user', ['qq'=>$qq, 'url'=>$url, 'keylogin'=>$keylogin, 'refund'=>$refund, 'transfer'=>$transfer, 'remain_money'=>$remain_money], ['uid'=>$uid]);
	}
	if($sqs!==false){
		exit('{"code":1,"msg":"succ"}');
	}else{
...
case 'statistics':
    ...
    SUM(CASE WHEN A.status = 4 THEN refundmoney ELSE 0 END) AS refundMoney
    ...
    SUM(CASE WHEN A.status = 4 THEN 1 ELSE 0 END) AS refundCount
    FROM pre_order A LEFT JOIN pre_channel B ON A.channel=B.id WHERE {$sql} order by trade_no desc");
